...
from scipy.ndimage import shift  # ghjk: scipy>=1.10 moved this out of .interpolation
...
# --- ghjk compatibility shim ------------------------------------------------
# numpy 1.x silently wrapped out-of-range values when constructing a fixed-width
# integer; numpy 2 raises OverflowError instead. The original code relies on
# C-style wraparound in its RNG and head-switching maths, so reproduce that
# explicitly rather than changing the arithmetic.
def _wrap_i32(v) -> numpy.int32:
    v = int(v) & 0xFFFFFFFF
    return numpy.int32(v - 0x100000000 if v >= 0x80000000 else v)


def _wrap_u32(v) -> numpy.uint32:
    return numpy.uint32(int(v) & 0xFFFFFFFF)
# --- end shim ---------------------------------------------------------------
...
class XorWowRandom:
    def __init__(self, seed1: int, seed2: int):
        self.x: int = _wrap_i32(seed1)
        self.y: int = _wrap_i32(seed2)
        self.z: int = numpy.int32(0)
        self.w: int = numpy.int32(0)
        self.v: int = _wrap_i32(-int(seed1) - 1)
        self.addend: int = _wrap_i32((int(seed1) << 10) ^ (int(_wrap_u32(seed2)) >> 4))
        [self._nextInt() for _ in range(0, 64)]

    def _nextInt(self) -> int:
        t = int(self.x)
        t = _wrap_i32(t ^ (int(_wrap_u32(t)) >> 2))
        self.x = _wrap_i32(self.y)
        self.y = _wrap_i32(self.z)
        self.z = _wrap_i32(self.w)
        v0 = _wrap_i32(self.v)
        self.w = _wrap_i32(v0)
        t = _wrap_i32((int(t) ^ (int(t) << 1)) ^ int(v0) ^ (int(v0) << 4))
        self.v = _wrap_i32(t)
        self.addend = _wrap_i32(int(self.addend) + 362437)
        return _wrap_i32(int(t) + int(self.addend))
    ...
    def nextInt(self, _from: int = Int_MIN_VALUE, until: int = Int_MAX_VALUE) -> numpy.int32:
        ...
        if n > 0 or n == Int_MIN_VALUE:
            if (n & -n) == n:
                ...
                v: int = 0
                while True:
                    bits = int(_wrap_u32(self._nextInt())) >> 1
                    v = bits % n
                    if bits - v + (n - 1) >= 0:
                        break
                return _wrap_i32(_from + v)
        else:
            r = range(_from, until)
            while True:
                rnd = self._nextInt()
...
class Ntsc:
    ...
    def vhs_head_switching(self, yiq: numpy.ndarray, field: int = 0):
        ...
        if self._vhs_head_switching_phase_noise != 0.0:
            x = _wrap_i32(self.rand() * self.rand() * self.rand() * self.rand())  # ghjk: wraparound
            x %= 2000000000
